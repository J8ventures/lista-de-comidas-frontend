import { api } from './api';
import { PlanComida, PlanComidaCrear, EntradaPlanComida, EntradaPlanCrear, ListaCompras } from '../types';

export const mealPlansService = {
  list: () =>
    api.get<PlanComida[]>('/planes-comida').then(r => r.data),

  get: (id: string) =>
    api.get<PlanComida>(`/planes-comida/${id}`).then(r => r.data),
