...
export const RecipeIngredientEditor: React.FC<RecipeIngredientEditorProps> = ({
  ...
  const updateAlt = (idx: number, altIdx: number, field: string, val: unknown) => {
    ...
    if (field === 'id_ingrediente' && ing) {
      next[idx].alternativas[altIdx] = {
        ...next[idx].alternativas[altIdx],
        id_ingrediente: val as string,
        unidad: ing.unidad,
      };
    } else {
  ...
  const ingredientOptions = ingredientes.map((i) => ({
    value: i.id,
        ...
        <div key={idx} className="border rounded-lg p-4 bg-gray-50">
          <div className="grid grid-cols-2 gap-3 mb-3">
            <Select
              ...
              onChange={(e) => update(idx, 'id_ingrediente', e.target.value)}
            ...
            <Input
              ...
              type="number"
              value={ri.cantidad}
              onChange={(e) => update(idx, 'cantidad', parseFloat(e.target.value))}
              min={0}
              step={0.1}
            />
            <Input
              ...
              {ri.alternativas.map((alt, altIdx) => (
                <div key={altIdx} className="grid grid-cols-3 gap-2 mb-2 items-end">
                  <Select
                    ...
                    onChange={(e) => updateAlt(idx, altIdx, 'id_ingrediente', e.target.value)}
                    options={ingredientOptions}
                  />
                  <Input
                    type="number"
                    value={alt.cantidad}
                    onChange={(e) => updateAlt(idx, altIdx, 'cantidad', parseFloat(e.target.value))}
                    min={0}
                    step={0.1}
                  />
                  <Button size="sm" variant="danger" onClick={() => removeAlt(idx, altIdx)}>
                    Eliminar
                  </Button>
                </div>
              ))}
              <Button size="sm" variant="ghost" onClick={() => addAlt(idx)}>
                + Agregar alternativa
              </Button>
            </div>
          )}
          ...
          <div className="flex justify-between mt-3">
            <Badge label={ri.rol} />
            <Button size="sm" variant="danger" onClick={() => remove(idx)}>
              Eliminar
            </Button>
          </div>
        </div>
