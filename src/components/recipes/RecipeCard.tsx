  ...
  <Card onClick={() => onView(receta.id)}>
    <div className="p-4 flex items-center justify-between">
      <div>
        ...
        {receta.descripcion && <p className="text-sm text-gray-500 mt-0.5 line-clamp-1">{receta.descripcion}</p>}
      ...
      <div className="flex gap-2" onClick={e => e.stopPropagation()}>
        <Button size="sm" variant="secondary" onClick={() => onView(receta.id)}>Ver</Button>
        <Button size="sm" variant="danger" onClick={() => onDelete(receta.id)}>Eliminar</Button>
      </div>
    </div>
  </Card>
);
